...
armory.get = function(realm,name,o,host,update,todo)
{
	...
	if (host=='cn')
		...
		var u = 'api/wow/character/'+e(realm)+'/'+e(name)+'?fields=guild,items,professions,stats,talents,titles,stats'
		}
	else if (host=='us')
		{
		...
		function(x){
			if (!x)
				...
				if(typeof(x[0])=='string')
					...
					try{
						o.innerHTML=self.proc_v3(x[0],realm,name,o,host,hosthost,iconbase,u,dbbase)
						}
					catch(err){
						console.log(err)
...
armory.proc_v3 = function(t,realm,name,o,host,hosthost,iconbase,u,dbbase)
{
if (!t.name)
	return this.error(3,realm,name,host,hosthost,u);
...
r = "\
		<div class='armorychr armorychr_v2' style='padding:0 0.25em 0 130px;margin-bottom:0.1em;background:#000 url("+t.thumbnail+") no-repeat -50px bottom;' class='"+style+"'>\
			<nobr> <a class='xtxt small_colored_text_btn' style='background:gray' href='javascript:void(0)' onclick='armory.get(\""+realm+"\",\""+name+"\",this.parentNode.parentNode.parentNode,\""+host+"\",1)' title='刷新 数据缓存可能需要数小时或数天更新'>#</a> <span class='xtxt subcolor'>("+host+")</span> "+_realm+"<span class='class"+t['class']+"'>[lv"+t.level+" "+this.race[t.race][0]+" "+specName+" "+this.className[t['class']][0]+"]</span></nobr> <span class='nobr'>[<a href='http://"+hosthost+"/wow/"+((host=='cn'||host=='tw') ? 'zh' : 'en')+"/character/"+encodeURIComponent(t.realm)+"/"+encodeURIComponent(t.name)+"/advanced' target='_blank' class='name'>"+_name+"</a>]</span> <br/>\
			"+talent+"\
...
for(var k in t.skills.active){
	...
	if(s.skill)
		r+=	"<a href='javascript:void(0)' onclick='d3armory.skillTip(event,\""+s.skill.tooltipUrl+"\",\""+s.rune.tooltipParams+"\")'><img src='http://media.blizzard.com/d3/icons/skills/21/"+s.skill.icon+".png' title='"+s.skill.name+"/"+s.rune.name+"' style='width:2em;height:2em;border:0.0833em solid #ca6d58'/></a> "
	}
