...
if (a.type=='js')
...
else
	{
		...
		{
			if (a['file'])
				var img = "<img id='"+id+"' src='"+a['file']+"' title='"+a['title']+"' style='border:1px solid #000000;"+(a['width']?' ;width:'+a['width']+'px ':'')+(a['height']?' ;height:'+a['height']+'px ':'')+"'"+log+"/>";
			else
				var img = a['title']
			if (a['url'])
