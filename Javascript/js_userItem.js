...
this._iList._.aC(_$('<h4/>')._.cls('textTitle')._.attr('innerHTML',
	(callerArg && callerArg[1] && callerArg[1].useArg) ? '请选择一个道具购买并使用' : 
		((data.ownerUid && data.ownerUid!=this._currentUid) ? '用户 '+data.ownerUname+' 的' : '') + '物品列表'
	))
...
if(!this._ui){
	...
	this._content = _$('<div/>')
	this._mainMenu = _$('<div/>')._.aC(
					_$('<a/>')._.attr({href:'javascript:void(0)',innerHTML:'[我的仓库]'})._.on('click',function(){self.list()}),
					_$('<a/>')._.attr({href:'javascript:void(0)',innerHTML:'[系统商店]'})._.on('click',function(){self.storeUi()}),
					_$('<a/>')._.attr({href:'javascript:void(0)',innerHTML:'[市场]'})._.on('click',function(){self.marketUi()}),
					_$('<a/>')._.attr({href:'javascript:void(0)',innerHTML:'[兑换码输入]'})._.on('click',function(){self.codeUi()})
					)
	this._ui = _$('<div/>')._.cls('item_ui')._.aC(
				this._mainMenu,
				this._content
				)
...
this._iAlert._.aC(
	...
	_$('<ul/>')._.aC(
		_$('<li/>')._.aC(
			document.createTextNode('输入兑换码'),
			_$('<input/>')._.attr({size:20}),
			_$('<a/>')._.attr({href:'javascript:void(0)',innerHTML:use?'[兑换并使用]':'[兑换]'})._.on('click',function(){
...
c._.addContent(
		...
		$('/br'),
		$('/br'),
		$('/button').$0('innerHTML','add','type','button','onclick',function(){
				self.medalProduce(this.parentNode.getElementsByTagName('input')[0].value, this.parentNode.getElementsByTagName('textarea')[0].value.replace("\n",',') )
				}
			),
